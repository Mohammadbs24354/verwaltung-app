    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Aktuelle Woche: KW{kw}</p>
        </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-gray-700 font-medium">Keine Dienstpläne gefunden</p>
            <p className="text-gray-500 text-sm mt-1">Erstellen Sie den ersten Dienstplan für {filterJahr}</p>
          </CardContent>
        </Card>
      ) : (
                    ...
                    <Link key={s.id} href={`/dashboard/dienstplaene/${s.id}`} className="flex items-center justify-between rounded-lg border border-gray-100 p-3 hover:bg-gray-50 transition-colors group">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-600">{s.entries?.length ?? 0} Einträge</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
      ...
      {showDialog && (
        <NeuerDienstplanDialog
          ...
          onCreated={(s) => { setSchedules((prev) => [s as Schedule, ...prev]); setShowDialog(false); }}
        />
      )}
    </div>
  );
