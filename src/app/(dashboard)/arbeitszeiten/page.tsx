  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500">Über- und Minusstunden auf einen Blick</p>
      </div>
                  ...
                  {data.entries.map((e) => {
                    ...
                    return (
                      <div key={e.id} className="flex items-center justify-between rounded-lg border border-gray-100 p-3">
                        <div>
                          <p className="text-sm font-medium">{format(new Date(e.datum + "T12:00:00"), "EEEE, dd.MM.yyyy", { locale: de })}</p>
                          <p className="text-xs text-gray-500">{e.schedule.filiale.name}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-semibold text-blue-700">{e.startZeit} – {e.endZeit}</p>
                          <p className="text-xs text-gray-500">{h.toFixed(2)}h netto</p>
                        </div>
                      </div>
                    );
                  })}
