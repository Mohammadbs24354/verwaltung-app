...
import { CheckCircle, XCircle, Send, FileDown, Plus, Pencil, Loader2, History } from "lucide-react";
import { getStatusColor, getStatusLabel, berechneStunden, formatDatum } from "@/lib/utils";
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500">{schedule.filiale.name}</p>
          {schedule.freigegebenAt && (
            <p className="text-xs text-gray-400 mt-1">Freigegeben am {format(new Date(schedule.freigegebenAt), "dd.MM.yyyy HH:mm", { locale: de })}</p>
          )}
        </div>
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          {canEdit && (
            <Button variant="outline" onClick={() => setShowEintragDialog(true)}>
                  ...
                  <TableBody>
                    {grouped[day].map((e) => (
                      <TableRow key={e.id}>
                        ...
                        <TableCell className="text-gray-500">{e.notiz ?? "–"}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
            ...
            <div className="space-y-3">
              {schedule.auditLogs.map((log) => (
                <div key={log.id} className="flex items-start gap-3 border-b border-gray-50 pb-3 last:border-0">
                  ...
                  <div>
                    <p className="text-sm font-medium">{log.user.name} <span className="font-normal text-gray-500">· {log.aktion}</span></p>
                    <p className="text-xs text-gray-400">{format(new Date(log.createdAt), "dd.MM.yyyy HH:mm", { locale: de })}</p>
      ...
      {showEintragDialog && (
        <EintragDialog
          ...
          onClose={() => setShowEintragDialog(false)}
          onSaved={() => {
